          ...
          <Collapsible open={isAdditionalInformationOpen} onOpenChange={setIsAdditionalInformationOpen}>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                {(() => {
                  const selectedIds = form.watch("relatedResearches")
                    .map(item => item.value)
              ...
              {relatedResearchFields.map((field, index) => {
                const selectedValue = form.watch(`relatedResearches.${index}.value`);
                const selectedResearch = allResearches.find(r => r.id.toString() === selectedValue);
                    ...
                    >
                      <SelectTrigger className="flex-1">
                        <SelectValue placeholder={t('research.relatedResearchesPlaceholder')}>
                          {selectedResearch ? `${selectedResearch.name} (${selectedResearch.team})` : t('research.relatedResearchesPlaceholder')}
                        </SelectValue>
                      </SelectTrigger>
                      <SelectContent>
                        {allResearches
                          .filter((r: Research) => {
                            ...
                            const selectedIds = form.watch("relatedResearches")
                              .map((item, idx) => idx !== index ? item.value : null)
                              .filter(id => id && id.trim() !== "");
